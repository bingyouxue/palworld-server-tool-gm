...
// SSEAuthMiddleware validates a JWT that can be supplied either as a standard
// Authorization: Bearer <token> header, or as a ?token=<token> query parameter.
// The query-param form is needed because the browser's EventSource API cannot
// send custom headers.
