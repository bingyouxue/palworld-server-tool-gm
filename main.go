...
//go:embed gm-data/items/* gm-data/pals/*
var gmData embed.FS
...
func main() {
	...
	gmDataFS, _ := fs.Sub(gmData, "gm-data")
	router.StaticFS("/gm-data", http.FS(gmDataFS))
	...
	go openBrowser(fmt.Sprintf("http://127.0.0.1:%d", settings.Web.Port))
...
func openBrowser(url string) {
	...
	if err := cmd.Start(); err != nil {
		logger.Errorf("Failed to open browser: %v\n", err)
	}
}
