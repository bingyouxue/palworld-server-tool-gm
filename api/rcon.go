...
//	@Description	Send Rcon Command by UUID (looks up command from DB, appends content)
//	@Tags			Rcon
//	@Accept			json
//	@Produce		json
//	@Security		ApiKeyAuth
...
// ExecRconRequest is a raw RCON command (no DB lookup required).
type ExecRconRequest struct {
	Command string `json:"command"`
}

// execRconCommand godoc
//
//	@Summary		Execute Raw Rcon Command
//	@Description	Execute an arbitrary RCON command string directly (no UUID lookup)
//	@Tags			Rcon
//	@Accept			json
//	@Produce		json
//	@Security		ApiKeyAuth
//	@Param			command	body		ExecRconRequest	true	"Raw command"
//	@Success		200		{object}	MessageResponse
//	@Failure		400		{object}	ErrorResponse
//	@Router			/api/rcon/exec [post]
func execRconCommand(c *gin.Context) {
	var req ExecRconRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
